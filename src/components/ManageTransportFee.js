...
const ManageTransportFee = () => {
    ...
    const fetchTransportFees = async () => {
        try {
            const response = await axios.get('http://localhost:3001/api/transportfees');
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:3001/api/transportfees/${id}`);
            setTransportFees(prevState => prevState.filter(fee => fee._id !== id));
    ...
    return (
     ...
     <div className="manage" style={{fontFamily: 'Poppins-Medium, sans-serif' }}>
            ...
            {transportFees.length > 0 ? (
                ...
                <p>No transport fees found.</p>
            )}
        </div>
    );
};

export default ManageTransportFee;
